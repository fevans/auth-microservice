...
public class JwtTokenService(IAuthStore authStore, AuthOptions options) : ITokenService
{
    private readonly string _issuer = options.AuthMicroserviceBaseAddress;

    public async Task<AuthToken?> GenerateAuthenticationToken(string username, string password)
    {
        var user = await authStore.VerifyUserLogin(username, password);
